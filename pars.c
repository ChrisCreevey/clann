...
{  /* Discrete character parsimony by uphill search */
	int argc; 
	char argv[10][100];
	char inputfilename[100], outputfilename[100], outputtreename[100];
  /* reads in spp, chars, and the data. Then calls maketree to
	...
	inputfilename[0] ='\0';
	 outputfilename[0] = '\0';
	 outputtreename[0] = '\0';
  strcpy(inputfilename, "coding.nex");
  strcpy(outputfilename, "clanntmp.chr");
  strcpy(outputtreename, "clanntree.chr");
  
  progname = argv[0];
  openfile(&infile,inputfilename,"input file", "r",argv[0],infilename);
  openfile(&outfile,outputfilename,"output file", "w",argv[0],outfilename);
  ...
  if (trout)
    openfile(&outtree,outputtreename,"output tree file", "w",argv[0],outtreename);
