...
double sup=1;
char saved_supertree[TREE_LENGTH],  *test_array, inputfilename[10000], delimiter_char = '.', logfile_name[10000];
        ...
        if(strcmp(parsed_command[i], "seed") == 0)
        	...
        	for(j=0; j<strlen(parsed_command[i+1]); j++)
      		...
      		if(isdigit==FALSE)
  				...
  				srand((unsigned) (seed));
  				printf("The seed value for the random number generator has been set to %d\n", seed);
  				}
        	}
        }
...
void do_log(void)
    ...
    for(i=0; i<num_commands; i++)
        ...
        if(strcmp(parsed_command[i], "file") == 0)
	     	...
	     	if((logfile = fopen(logfile_name, "w")) == NULL)
				{
				printf("Error opening log file named %s\n", logfile_name);
				error = TRUE;
				}
			else
        ...
        if(strcmp(parsed_command[i], "status") == 0)
	        ...
	        if(strcmp(parsed_command[i+1], "on") == 0)
		        ...
		        if(print_log == FALSE)
			        ...
			        printf("starting logging to logfile %s\n", logfile_name );
			        print_log = TRUE;
			        start=TRUE;
			    	}
			    else
			    	{
			    	printf("Already logging output, no changes have been made\n");
			    	}
			    }
			else
				{
			    if(strcmp(parsed_command[i+1], "off") == 0)
			        ...
			        if(print_log == TRUE)
			        	{	
			        	printf("stopping logging to file %s\n", logfile_name);
			        	print_log = FALSE;
			        	stop=TRUE;
			        	}
			        else
			        	{	
			        	printf("Cannot stop logging as there is no logging currently happenning\n");
			        	}
			        }
			    else
			    	{
			    	printf("Error: \"%s\" is not a valid modifer for option \"status\", please choose \"on\" or \"off\"", parsed_command[i+1]);
			    	}	
			    }
		    }
	...
	if(newlogfile==FALSE && start==FALSE && stop == FALSE)
		...
		if(logfile!= NULL) 
			{
			printf("log file \"%s\" open ", logfile_name);
			if(print_log == TRUE)
				printf("and logging is currently on)\n");
			else
				printf("and logging is currently off)\n");
