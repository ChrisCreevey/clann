...
void treeout3(node *p, long nextree, long *col, node *root)
{
  ...
  if (p != root) {
	...
	length[0] = '\0';
	sprintf(length, ":%*.2f", (int)(w + 4), x);
	strcat(test_array, length);
	testarraypos+=strlen(length);
 ...
 test_array[testarraypos] = '\0';
 strcpy(retained_supers[num_best_trees_found], test_array);
 num_best_trees_found++;
 if(num_best_trees_found+1 == number_retained_supers)
